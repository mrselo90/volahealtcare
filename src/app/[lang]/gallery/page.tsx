...
export default function GalleryPage() {
  ...
  const fetchTestimonials = async () => {
    try {
      const response = await fetch('/api/testimonials?approved=true&featured=true&limit=3');
      if (response.ok) {
  ...
  const defaultTestimonials = [
    {
      ...
      country: 'Germany',
      rating: 5,
      review: 'My hair transplant operation was perfect. Natural-looking results and professional service. I definitely recommend it.',
      service: { title: 'Hair Transplant', slug: 'hair-transplant' }
    },
    {
      id: '2',
      name: 'Sarah D.',
      country: 'UK',
      rating: 5,
      review: 'My dental treatment went great. My smile completely changed and my self-confidence increased. Thank you Vola Health!',
      service: { title: 'Dental Treatment', slug: 'dental-treatment' }
    },
    {
      id: '3',
      name: 'Emma S.',
      country: 'USA',
      rating: 5,
      review: 'My plastic surgery operation exceeded my expectations. Natural results and excellent care. I recommend it to everyone.',
      service: { title: 'Plastic Surgery', slug: 'plastic-surgery' }
    }
  ];
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {testimonialsLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[1, 2, 3].map((i) => (
                <div key={i} className="bg-white/10 backdrop-blur-sm p-6 rounded-2xl border border-white/20 animate-pulse">
                  <div className="flex items-center mb-4">
                    <div className="flex space-x-1">
                      {[...Array(5)].map((_, j) => (
                        <div key={j} className="w-5 h-5 bg-gray-400 rounded"></div>
                      ))}
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {displayTestimonials.map((testimonial, index) => (
                <div key={testimonial.id} className="bg-white/10 backdrop-blur-sm p-6 rounded-2xl border border-white/20">
                  <div className="flex items-center mb-4">
                    <div className={`flex ${index === 0 ? 'text-blue-400' : index === 1 ? 'text-purple-400' : 'text-blue-500'}`}>
                  ...
                  <div className="text-sm text-gray-400">{testimonial.service.title} Patient • {testimonial.country}</div>
                </div>
              ))}
            </div>
          )}
